...
def compute_occupancy_stats(flt) -> Dict[str, Any]:
    ...
    atelier_ids = [int(aid) for aid in (getattr(flt, "atelier_ids", None) or []) if aid not in (None, "", "None")]
    if not atelier_ids and getattr(flt, "atelier_id", None):
        atelier_ids = [int(flt.atelier_id)]
    if atelier_ids:
        q = q.filter(SessionActivite.atelier_id.in_(atelier_ids))
