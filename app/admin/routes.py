...
def _roles_sector_policy(roles: list[Role]) -> dict[str, dict[str, bool]]:
    """Métadonnées UI pour savoir si un rôle doit pouvoir porter un secteur assigné.

    Règle simple et sûre : un rôle global (permission scope:all_secteurs) n'a pas besoin
    d'un secteur assigné. Tous les autres peuvent en recevoir un.
    """
    out: dict[str, dict[str, bool]] = {}
    for role in roles:
        perm_codes = {p.code for p in getattr(role, "permissions", []) or []}
        is_global = "scope:all_secteurs" in perm_codes
        out[role.code] = {
            "allow_sector": not is_global,
            "is_global": is_global,
        }
    return out
...
def users():
    ...
    return render_template(
        ...
        roles=roles,
        secteurs=secteurs,
        role_sector_policy=_roles_sector_policy(roles),
    )
